...
def create_llm_client(
    ...
    provider = str(
        args.get("provider") or values.get("MOFA_NOTEBOOK_PROVIDER") or values.get("MOFA_DATA_TABLE_PROVIDER") or ""
    ).strip().lower()
    ...
    if provider not in {"gemini", "openai", "vertex"}:
        raise ValueError(f"Unsupported notebook model provider: {provider}")

    model_override = str(
        args.get("model") or values.get("MOFA_NOTEBOOK_MODEL") or values.get("MOFA_DATA_TABLE_MODEL") or ""
    ).strip()
    if provider == "gemini":
        if not gemini_key:
