...
Model-specific endpoints (each model type has its own URL for clarity):
...
**API route:** `POST /v1/audio/asr/qwen3`

### voice_synthesize

Generate speech audio from text via Qwen3-TTS. Produces a WAV file. The `speaker` parameter accepts both preset names and saved voice profile names.
...
**API route:** `POST /v1/audio/tts/qwen3?format=wav`
...
**API route:** `POST /v1/audio/tts/clone?format=wav`
