...
class NotebookMindmapTests(unittest.TestCase):
    ...
    def test_main_runs_when_installed_without_repo_common(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            workspace.mkdir()
            installed_skill = root / "installed-skills" / "mofa-notebook-mindmap"
            installed_skill.parent.mkdir()
            shutil.copytree(ROOT / "mofa-notebook-mindmap", installed_skill)

            completed = subprocess.run(
                [str(installed_skill / "main"), "mindmap_generate"],
