...
2. **Language match end-to-end.** If the user writes Chinese, EVERYTHING inside `prompt` strings is Chinese — VQA block, layout instructions (`主标题(粗体)`, NOT `TITLE:`), card labels, narratives, and the quoted text-to-render. The ONLY English allowed is established proper nouns (Linux, Gemini, MCP, etc.). Mixing English instructional headers like `VISUAL:` / `TITLE:` / `Elements:` with Chinese content is the #1 cause of 乱码 in Gemini's image output — see `docs/cc-ppt-authoring.md` anti-pattern. Highest-impact rule for Chinese decks. For Chinese decks also pass `image_size: "4K"` (default 2K is too low for clean stroke rendering).
