...
- Your prompt must explicitly leave clean space for the overlays — describe the background only (decorations, illustration zones, atmosphere) and tell Gemini where to leave whitespace. The runtime no longer auto-appends a "do not render text" clamp, so write that requirement into the per-deck VQA block yourself (see "Authoring a deck — required cc-ppt structure" below).
...
Every deck is its own self-contained script. There is no shared "template" file you can reuse across decks — the deck IS the script. The script has three parts in this exact order:

1. **Top constants** — `SLIDE_DIR`, `OUT_FILE`, `IMAGE_SIZE`, `MODEL`, `CONCURRENCY`. Documentation for the human author; the `mofa_slides` tool reads the matching values off its own arguments, not the script.
2. **A `const VQA = \`...\`` block in the deck's primary language** — strict visual-quality rules: no page numbers, no leaked formatting hints, language-specific font/punctuation rules. The VQA is in the language of the deck (Chinese VQA for Chinese decks, English VQA for English decks). The runtime does NOT inject any "do not render text" clamp anymore — if you want a clean background, write that rule into the VQA block yourself, in the deck's language.
...
- Write `script.js` to the workspace first with the `write_file` tool — do NOT pass an inline `slides: [...]` array to `mofa_slides` anymore. Always invoke with `input: "<workspace-relative path to script.js>"` after the file is on disk.
...
- Reference (full real-world example): see `cc-ppt/generate-westlake-engine.js` for a 477-line production deck — same structure, just a `run()` invocation instead of `module.exports`.
...
Then invoke: `mofa_slides({ input: "slides/<slug>/script.js", style: "fengzikai", out: "slides/<slug>/output/deck.pptx", slide_dir: "slides/<slug>/output/imgs", image_size: "2K", concurrency: 5 })`.
...
Then invoke: `mofa_slides({ input: "slides/<slug>/script.js", style: "nb-pro", out: "slides/<slug>/output/deck.pptx", slide_dir: "slides/<slug>/output/imgs", image_size: "2K", concurrency: 5 })`.

### Deprecated: inline `slides: [...]` argument

Passing `slides: [...]` directly to `mofa_slides` still works for backward compatibility, but is deprecated. Always prefer writing a script file and passing `input: "<path>"`. Reasons:

- The script lives in the workspace, so the user can iterate on it without re-prompting you.
- A per-deck `const VQA` block keeps language matching honest end-to-end; inline arrays tend to drop VQA entirely.
- Template-splicing `${VQA}` is awkward inside an inline JSON array and easy to forget.
...
1. Generate the illustrated background from your prompt (no "do not render text" clamp is auto-appended anymore — author the no-text rule into your per-deck VQA block, see "Authoring a deck — required cc-ppt structure" below)
